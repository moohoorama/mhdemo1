...
func sample(mask int, u, v float64) float64 {
	...
	// Resolve diagonal saddles with connected land at the center. This term is
	// zero on every edge, so the shared-edge contract is preserved.
	if mask == 5 || mask == 10 {
		s += .18 * 16 * u * (1 - u) * v * (1 - v)
	}
	return s
...
// asset order is eight water frames, soil plus fourteen edges, then grass plus
// fourteen edges. The two unused atlas cells remain transparent.
